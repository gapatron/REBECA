...
def main():
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        if accelerator.is_main_process:
            ...
            if args.validation_prompt is not None and epoch % args.validation_epochs == 0:
                ...
                # create pipeline
                # 1) Build a fresh base pipeline in the desired dtype
                ...
                # 2) Re-create LoRA adapters on this *fresh* pipeline and copy weights from the trained models
                UNET_TARGET_MODULES = ["to_q", "to_k", "to_v", "to_out.0"]
                TEXT_ENCODER_TARGET_MODULES = ["q_proj", "k_proj", "v_proj", "out_proj"]
                ...
                unet_sd = get_peft_model_state_dict(unwrapped_unet, state_dict=accelerator.get_state_dict(unet))
                set_peft_model_state_dict(pipe.unet, unet_sd)

                if args.train_text_encoder:
                    te_cfg = LoraConfig(
                        r=args.lora_text_encoder_r, lora_alpha=args.lora_text_encoder_alpha,
                        lora_dropout=args.lora_text_encoder_dropout, bias=args.lora_text_encoder_bias,
                        target_modules=TEXT_ENCODER_TARGET_MODULES,
                    )
                    pipe.text_encoder = get_peft_model(pipe.text_encoder, te_cfg)
                    te_sd = get_peft_model_state_dict(unwrapped_te, state_dict=accelerator.get_state_dict(text_encoder))
                    set_peft_model_state_dict(pipe.text_encoder, te_sd)

                # 3) Make sure adapters are in the same dtype as the rest
    ...
    if accelerator.is_main_process and args.use_peft:
        ...
        # Use the same target module lists you used during training
        UNET_TARGET_MODULES = ["to_q", "to_k", "to_v", "to_out.0"]
        TEXT_ENCODER_TARGET_MODULES = ["q_proj", "k_proj", "v_proj", "out_proj"]

        unet_peft = accelerator.unwrap_model(unet)
        unet_sd = get_peft_model_state_dict(unet_peft, state_dict=accelerator.get_state_dict(unet))
        save_file(unet_sd, os.path.join(args.output_dir, f"{global_step}_unet_peft.safetensors"))

        te_cfg = None
        if args.train_text_encoder:
            te_peft = accelerator.unwrap_model(text_encoder)
            te_sd = get_peft_model_state_dict(te_peft, state_dict=accelerator.get_state_dict(text_encoder))
            save_file(te_sd, os.path.join(args.output_dir, f"{global_step}_te_peft.safetensors"))
            te_cfg = {
                "r": args.lora_text_encoder_r,
                "lora_alpha": args.lora_text_encoder_alpha,
                "lora_dropout": args.lora_text_encoder_dropout,
        ...
        with open(os.path.join(args.output_dir, f"{global_step}_peft_config.json"), "w") as f:
            json.dump(cfg, f)

    # Final inference
    # Load previous pipeline
    # Final inference (load LoRA via Diffusers)
    from diffusers import StableDiffusionPipeline
    ...
    with open(os.path.join(args.output_dir, f"{global_step}_peft_config.json"), "r") as f:
        cfg = json.load(f)

    # UNet adapter
    unet_cfg = LoraConfig(**cfg["unet"])
    pipeline.unet = get_peft_model(pipeline.unet, unet_cfg)
    unet_sd = load_file(os.path.join(args.output_dir, f"{global_step}_unet_peft.safetensors"))
    ...
    if cfg["text_encoder"] is not None:
        ...
        te_sd = load_file(os.path.join(args.output_dir, f"{global_step}_te_peft.safetensors"))
